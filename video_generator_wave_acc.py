...
Génère une vidéo podcast avec visualisation audio réactive ET accélération de l'audio à 1.25x.

Ce script combine les fonctionnalités de video_generator_wave.py avec l'accélération audio.
Il prend en entrée un fichier audio/vidéo et génère une vidéo avec:
- Vidéo de fond en boucle
- Visualisation audio réactive (waveform customisable)
- Audio accéléré à 1.25x la vitesse originale

Dossiers:
- working_dir_wave/: Contient background_video.mp4 et la source audio/vidéo
- video_generator_wave_output/: Sortie des vidéos générées (suffixe _Acc.mp4)
"""
...
SPEED_FACTOR = 1.25  # Accélération à 1.25x
...
def accelerate_audio(input_audio_path, output_audio_path, speed_factor=SPEED_FACTOR):
    """
    ...
    Args:
        ...
        speed_factor: Facteur d'accélération (ex: 1.25 = 1.25x plus rapide)
    """
...
def main(style=DEFAULT_STYLE, position="bottom", color="white", opacity=DEFAULT_OPACITY):
    ...
    waveform_video = generate_waveform_video(accelerated_audio, waveform_video_path, style, color, opacity)
    ...
    final_output_path = os.path.join(OUTPUT_DIR, f"podcast_wave_{input_basename}_{style}_{timestamp}_Acc.mp4")
    final_video = overlay_waveform_on_video(looped_video, waveform_video, accelerated_audio, final_output_path, position)
    ...
    print(f"📍 Position : {position}")
    print(f"⚡ Audio accéléré : {SPEED_FACTOR}x")
